...
// mongodb+srv://Tomas:TomasSanjiao149@cluster0.nyiuezw.mongodb.net/?retryWrites=true&w=majority
mongoose.connect(
  "mongodb+srv://nycolop:1bNFBpF3sQACNE6r@cluster0.njmcx5z.mongodb.net/?retryWrites=true&w=majority"
);
...
server.get("/api/class", async function (request, response) {
  ...
  for (let index = 0; index < clases.length; index++) {
    ...
    let intPayload = {};
    intPayload.clase = clase;

    intPayload.profesor = await Profesor.findOne({ _id: clase.profesorId });
    intPayload.user = await User.findOne({ _id: intPayload.profesor.userId });

    let ratingAverage = 0;
    for (let x = 0; x < clase.rating.length; x++) {
      ratingAverage += clase.rating[x].value;
    }
    ratingAverage = ratingAverage / (clase.rating.length || 1);

    await Class.updateOne({ _id: clase._id }, { $set: { ratingAverage } });
    payload.push(intPayload);
  }
...
server.post("/api/register", async function (request, response) {
  try {
    const { username, email, password, name, surname, role, phone } =
      request.body;
...
server.get("/api/password-recovery", async function (request, response) {
  ...
  const apiKey =
    "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";
